...
extension PositionCoordinator: UWBManagerDelegate{
    ...
    func didUpdatePosition(deviceId: String, distance: Double) {
        ...
        print("[UWBPlugin] Connected to \(self.anchors.count) sensors.")
        
        // 3. We compute trilateration
        if self.anchors.count < 3{
            print("[UWBPlugin] Connected to less than 3 sensors setting position to null.")
            self.filteredPos = nil // if weare connected to less than two sensors we return nil (i.e. no connection)
            return
        }
        
        print("[UWBPlugin] Connected to more than 3 sensors computing position.")
